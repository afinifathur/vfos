...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold text-white mt-4">Edit Budget</h2>
        <p class="text-slate-400 text-sm mt-1">Modify your spending limit for this category or classification.</p>
    </div>

    <div class="bg-card-dark border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <form action="{{ route('budgets.update', $budget) }}" method="POST" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="allocated_amount" id="allocated_amount" value="{{ old('allocated_amount', (int)$budget->allocated_amount) }}" required min="0" step="1000"
...
<script>
    ...
    window.onload = function() {
        ...
        if (initialCategoryId) {
            updateSubcategories();
            // Pre-select the existing subcategory
            const selectedSubId = "{{ old('subcategory_id', $budget->subcategory_id) }}";
            if (selectedSubId) {
                document.getElementById('subcategory_id').value = selectedSubId;
            }
        } else {
