                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                        ...
                        <div class="md:col-span-4 p-6 rounded-2xl bg-slate-900/40 border border-slate-800/50">
                            <div class="flex flex-col md:flex-row md:items-end gap-6">
                                ...
                                <div id="destination_account_wrapper" class="flex-1 space-y-2 hidden transition-all duration-300">
                                    ...
                                    <input type="hidden" name="to_account_id" id="to_account_id" value="{{ old('to_account_id', $transaction->to_account_id) }}">
                                    <div class="relative" id="to_account_dropdown">
                                        ...
                                        <button type="button" id="to_account_trigger"
                                            ...
                                            <span id="to_account_label" class="flex-1 text-left truncate"
                                                @if(old('to_account_id', $transaction->to_account_id))
                                                    style="color:white"
                                                @else
                                                    style="color:#94a3b8"
                                                @endif>
                                                @php
                                                    $selectedToId = old('to_account_id', $transaction->to_account_id);
                                                    $selectedToName = $selectedToId
                                                        ? ($accounts->firstWhere('id', $selectedToId)?->name ?? 'Select Destination')
                                                        : 'Select Destination';
                                                @endphp
                                                {{ $selectedToName }}
                                            </span>
                                        ...
                                        <div id="to_account_panel"
                                            ...
                                            <div class="max-h-56 overflow-y-auto custom-scrollbar">
                                                ...
                                                <button type="button"
                                                    ...
                                                    class="to-account-option w-full flex items-center gap-3 px-4 py-1.5 text-left text-sm font-medium text-slate-300 hover:bg-primary/10 hover:text-white transition-colors {{ old('to_account_id', $transaction->to_account_id) == $account->id ? 'bg-primary/20 text-white font-bold' : '' }}">
                        ...
                        <div class="md:col-span-2 space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <div id="total_display" class="w-full pl-12 pr-4 py-4 bg-slate-800/60 border border-slate-700 rounded-xl text-white font-black text-3xl cursor-default select-none">
                                    {{ number_format(old('total_amount', (float)$transaction->total_amount), 0, ',', '.') }}
                                </div>
                                <input type="hidden" name="total_amount" id="total_amount" value="{{ old('total_amount', (float)$transaction->total_amount) }}">
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleTransferUI(type) {
            if (type === 'transfer') {
                ...
                document.getElementById('to_account_label').style.color = '#94a3b8';
                document.querySelectorAll('.to-account-option').forEach(btn => btn.classList.remove('bg-primary/20', 'text-white', 'font-bold'));
            }
        }
