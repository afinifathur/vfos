...
<div class="max-w-3xl mx-auto space-y-6">
    <header class="flex items-center gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        ...
        <div>
            <h2 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Edit Subcategory</h2>
            <p class="text-slate-500 dark:text-slate-400 mt-1">Update your classification details.</p>
        </div>
    </header>

    <div class="bg-white dark:bg-card-dark rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <form action="{{ route('subcategories.update', $subcategory) }}" method="POST" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <!-- Parent Category -->
                <div class="space-y-2">
                    <label for="category_id" class="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Parent Category</label>
                    <div class="relative">
                    ...
                    @error('category_id')<p class="text-xs text-rose-500 font-medium">{{ $message }}</p>@enderror
                </div>

                <!-- Name -->
                <div class="space-y-2">
                    <label for="name" class="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Subcategory Name</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $subcategory->name) }}" required
                        ...
                        placeholder="e.g. Electricity, Snacks, etc.">
            ...
            <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-500">
                        <span class="material-symbols-outlined">{{ $subcategory->is_active ? 'toggle_on' : 'toggle_off' }}</span>
                    </div>
                    <div>
                        <p class="text-sm font-bold text-slate-900 dark:text-white">Active Status</p>
                        <p class="text-xs text-slate-500">Inactive subcategories won't show in item selections.</p>
                    </div>
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $subcategory->is_active ? 'checked' : '' }}>
